...
def LoadDevice():
    try:
        ...
        h = hid.device()
        h.open(0xDA9A, 0x0101)  # TREZOR VendorID/ProductID
...
#01	04	10	18	00	02	F5	0C	00
    #读取第一通道的温度，单路usb只有第一通道
    #寄存器地址0x1018,读取两个字（温度数据占用两个字）
    cmd = [0x01, 0x04, 0x10, 0x18, 0x00, 0x02, 0xF5, 0x0C]
    ...
    if (dcrc == crc):
        value = GetIntFromBigEndianArray(resp, 3)
        # 读取到温度通道的寄存器进行大小端转化后
